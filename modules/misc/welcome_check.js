...
module.exports = async (client) => {
    ...
    setInterval(async () => {
        if (newUsers.size > 0) {
            ...
            if (newUsers.size === 1) {
                generalChan.send({
                    content: `We have a new friend! <:squee:838443107988799498> Welcome to the server <@${Array.from(newUsers).join('>, <@')}>! :wave: Feel free to <#1049263519255777301>, or just join the chat :slight_smile:`
                ...
                generalChan.send({
                    content: `We have some new friends! <:squee:838443107988799498> Welcome to the server <@${Array.from(newUsers).join('>, <@')}>! :wave: Feel free to <#1049263519255777301>, or just join the chat :slight_smile:`
                }).catch(err => console.error(`${path.basename(__filename)} There was a problem sending a message: `, err)).then(message => {
                    setTimeout(() => {
                        message.delete().catch(err => console.error(`${path.basename(__filename)} There was a problem deleting a message: `, err));
                    }, 300000);
