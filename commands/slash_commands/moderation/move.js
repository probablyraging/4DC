...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!guild.me.permissionsIn(channel).has("MANAGE_MESSAGES") || !guild.me.permissionsIn(channel).has("SEND_MESSAGES") || !guild.me.permissionsIn(channel).has("VIEW_CHANNEL")) {
            await interaction.editReply(`${process.env.BOT_DENY} \`I do not have the proper permissions for #${channel.name}\``)
        ...
        if (channelType !== "GUILD_TEXT") {
            await interaction.editReply(`${process.env.BOT_DENY} \`You can't move a message to a ${cType}\``)
                .catch(err => console.error(`${path.basename(__filename)} There was a problem sending an interaction: `, err));
            return;
        }
                ...
                if (msg.id === filteredArr[i]) {
                    ...
                    if (!author) {
                        ...
                        if (msgAttachment) {
                            ...
                            toChannel.createWebhook(authorUsername, {avatar: msg.author.displayAvatarURL()}).then(webhook => {
                                webhook.send({
                                    content: `${msgContent}`,
                                    files: imageArr
